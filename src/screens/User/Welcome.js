...
const Welcome = () => {
    ...
    return (
        <div className='welcome-main center'>
            <div className={`welcome-block `} style={{ backgroundColor: getBgColor() }}>
                <UserNavBar title={"home"} />

                <div className='profile-body'>
                    <img className="user-image" src={UserAvatar} alt="user-avatar" />
                    <span  data-testid="user-welcome-span"> {`${translate('welcome')} ${user?.name || ''}`}</span>
                    <span  data-testid="user-selected-lang-span"> Your selected language is : {lang}</span>
